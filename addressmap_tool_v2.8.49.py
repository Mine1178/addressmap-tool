...
class App:
            ...
            if mode == "district":
                ...
                if truncated and self.poi_break_limit.get():
                    ...
                    grid_pois, grid_calls = grid_collect_poi(gkey, clng, clat, kw, radius, lambda: self.poi_stop_flag,
                                                                   log_cb=lambda m: self.root.after(0, lambda mm=m: self.poi_log.insert(tk.END, mm + "\n", "info")), known=raw_results, workers=self.cfg.get("grid_workers", 3))
                    gaode_count += grid_calls
                    added = 0
                    for gp in grid_pois:
                        if not poi_idx.near_any(gp["gcj_lng"], gp["gcj_lat"]):
                        ...
                        if etruncated and self.poi_break_limit.get():
                            ...
                            if elng and elat:
                                ...
                                epois_g, ecalls = grid_collect_poi(gkey, elng, elat, ek, e_radius, lambda: self.poi_stop_flag,
                                                   log_cb=lambda m: self.root.after(0, lambda mm=m: self.poi_log.insert(tk.END, mm + "\n", "info")), poly=epoly, known=raw_results, workers=self.cfg.get("grid_workers", 3))
                                gaode_count += ecalls
                                eadd = 0
                                for gp in epois_g:
                                    if not poi_idx.near_any(gp["gcj_lng"], gp["gcj_lat"]):
    ...
    def _build_settings(self):
        ...
        tk.Label(r3, text="深挖并发数：", font=("微软雅黑", 11), bg=Config.C_CARD, fg=Config.C_TEXT).pack(side=tk.LEFT, padx=(30, 0))
        self.cfg_gw = tk.Spinbox(r3, from_=1, to=5, increment=1, width=6, font=("微软雅黑", 10), relief=tk.SOLID, bd=1)
        self.cfg_gw.delete(0, tk.END); self.cfg_gw.insert(0, str(int(self.cfg.get("grid_workers", 3)))); self.cfg_gw.pack(side=tk.LEFT, padx=10)
        ...
        aic = Card(f, title="AI 大模型智能增强（L0 · 可选）", accent=Config.C_INFO); aic.pack(fill=tk.X, pady=6)
        ...
        tk.Label(ra, text="模型平台：", font=("微软雅黑", 11), bg=Config.C_CARD, fg=Config.C_TEXT, width=15, anchor="e").pack(side=tk.LEFT)
        self.cfg_ai_prov = ttk.Combobox(ra, values=list(Config.AI_PROVIDERS.keys()) + ["custom"], font=("微软雅黑", 10), width=15, state="readonly")
        self.cfg_ai_prov.set(self.cfg.get("ai_provider", "doubao")); self.cfg_ai_prov.pack(side=tk.LEFT, padx=10)
        tk.Label(ra, text="模型：", font=("微软雅黑", 11), bg=Config.C_CARD, fg=Config.C_TEXT).pack(side=tk.LEFT, padx=(10, 0))
        self.cfg_ai_model = tk.Entry(ra, font=("微软雅黑", 10), width=26, relief=tk.SOLID, bd=1); self.cfg_ai_model.pack(side=tk.LEFT, padx=10)
        ...
        tk.Label(rb, text="Base URL：", font=("微软雅黑", 11), bg=Config.C_CARD, fg=Config.C_TEXT, width=15, anchor="e").pack(side=tk.LEFT)
        self.cfg_ai_base = tk.Entry(rb, font=("微软雅黑", 10), width=55, relief=tk.SOLID, bd=1); self.cfg_ai_base.pack(side=tk.LEFT, padx=10, fill=tk.X, expand=True)
        ...
        tk.Label(rc, text="API Key：", font=("微软雅黑", 11), bg=Config.C_CARD, fg=Config.C_TEXT, width=15, anchor="e").pack(side=tk.LEFT)
        self.cfg_ai_key = tk.Entry(rc, font=("微软雅黑", 10), width=55, show="*", relief=tk.SOLID, bd=1); self.cfg_ai_key.pack(side=tk.LEFT, padx=10, fill=tk.X, expand=True)
        ...
        for t in ["• 支持 OpenAI 兼容协议的模型平台：豆包（火山方舟）/ DeepSeek / 通义千问 / Kimi / 智谱 GLM / OpenAI 等",
            ...
            tk.Label(aic, text=t, font=("微软雅黑", 10), bg=Config.C_CARD, fg=Config.C_TEXT_M).pack(anchor="w", padx=15, pady=2)
    ...
    def _save_cfg(self):
        ...
        except: self.cfg["grid_workers"] = 3
