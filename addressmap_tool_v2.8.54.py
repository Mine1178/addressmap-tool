...
class App:
    ...
    def _build_settings(self):
        ...
        for t in ["• 高德 Key 为必填项，请前往 https://console.amap.com 申请个人开发者 Key",
            ...
            tk.Label(tip, text=t, font=("微软雅黑", 10), bg=Config.C_CARD, fg=Config.C_TEXT_M).pack(anchor="w", padx=16, pady=3)
        ...
        for t in ["• 支持 OpenAI 兼容协议：豆包 / DeepSeek / 通义 / Kimi / 智谱 GLM / OpenAI",
            ...
            tk.Label(aic, text=t, font=("微软雅黑", 10), bg=Config.C_CARD, fg=Config.C_TEXT_M).pack(anchor="w", padx=16, pady=3)
        ...
        tk.Label(mc, text="选择平台后自动填充 Base URL 与模型名；如需其他平台，选择 custom 手动填写。",
                 font=("微软雅黑", 10), bg=Config.C_CARD, fg=Config.C_TEXT_M).pack(anchor="w", padx=16, pady=(4, 2))
        for _gt in ["• 日常推荐：豆包 / DeepSeek / 通义（国内直连、延迟低）",
                    "• OpenAI 需海外网络，网络不稳时建议优先使用国内平台",
                    "• Key 仅保存在本机 addr_config.json 中，不会上传云端",
                    "• 平台更新模型名后，可选择 custom 手动填写新模型"]:
            tk.Label(mc, text=_gt, font=("微软雅黑", 10), bg=Config.C_CARD, fg=Config.C_TEXT_M).pack(anchor="w", padx=16, pady=1)
