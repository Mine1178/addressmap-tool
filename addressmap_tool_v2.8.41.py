...
def _parse_polyline(pl):
    """解析高德行政区 polyline（"lng,lat;lng,lat;..."）为点列表"""
    if not pl:
        return []
    pts = []
...
def grid_collect_poi(gkey, clng, clat, kw, radius, stop_check=None, log_cb=None, poly=None):
    """网格化周边扫描：以区县中心为原点，8 方位 + 中心共 9 个采样点，
    每点做周边搜索（单点翻页上限 9 页/225 条），合并返回全部 POI，
    用于突破高德单参数 225 条翻页限制、取全区县数据。
    ...
    for dx, dy in offsets:
        ...
        while not stop() and page <= 9:
            url = f"{Config.GAODE_POI_AROUND}?key={gkey}&location={c_lng},{c_lat}&keywords={url_enc(kw)}&radius={int(d)}&offset=25&page={page}&extensions=all&output=json"
            ok, resp = _http_retry(url, 2, 2)
            if not ok:
                break
            calls += 1
                ...
                for unit_info in units:
                    ...
                    if truncated and self.poi_break_limit.get():
                        ...
                        if mclng and mclat:
                            ...
                            grid_pois, grid_calls = grid_collect_poi(gkey, mclng, mclat, kw, grid_radius, lambda: self.poi_stop_flag, poly=poly)
                    ...
                    for unit_info in (units if mode == "district" else [{"name": ""}]):
                        ...
                        if etruncated and self.poi_break_limit.get():
                            ...
                            if elng and elat:
                                ...
                                epois_g, ecalls = grid_collect_poi(gkey, elng, elat, ek, e_radius, lambda: self.poi_stop_flag, poly=epoly)
                                gaode_count += ecalls
                                eadd = 0
                                for gp in epois_g:
                                    if not poi_idx.near_any(gp["gcj_lng"], gp["gcj_lat"]):
