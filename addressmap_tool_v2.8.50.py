...
class App:
    ...
    def _build_settings(self):
        ...
        col_l = tk.Frame(cols, bg=Config.C_BG); col_l.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 6))
        col_r = tk.Frame(cols, bg=Config.C_BG); col_r.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(6, 0))
        ...
        for lbl, attr, val in [("* 高德 API Key：", "cfg_gaode", self.cfg.get("gaode_key","")), ("  百度 AK：", "cfg_baidu", self.cfg.get("baidu_ak",""))]:
            ...
            tk.Label(r, text=lbl, font=("微软雅黑", 11), bg=Config.C_CARD, fg=Config.C_TEXT, width=13, anchor="e").pack(side=tk.LEFT)
            e = tk.Entry(r, font=("微软雅黑", 11), width=40, show="*", relief=tk.SOLID, bd=1); e.pack(side=tk.LEFT, padx=8, fill=tk.X, expand=True, ipady=0)
            e.insert(0, val); setattr(self, attr, e)
            tk.Button(r, text="显示/隐藏", font=("微软雅黑", 10), bg=Config.C_BORDER, fg=Config.C_TEXT, bd=0, padx=8, pady=4,
        ...
        tk.Label(r3, text="百度触发等级：", font=("微软雅黑", 11), bg=Config.C_CARD, fg=Config.C_TEXT, width=13, anchor="e").pack(side=tk.LEFT)
        self.cfg_trig = ttk.Combobox(r3, values=["门址","街道","乡镇","区县","城市"], font=("微软雅黑", 10), width=12, state="readonly")
        self.cfg_trig.set(self.cfg.get("baidu_trigger_level", "乡镇")); self.cfg_trig.pack(side=tk.LEFT, padx=8)
        tk.Label(r3, text="请求间隔（秒）：", font=("微软雅黑", 11), bg=Config.C_CARD, fg=Config.C_TEXT).pack(side=tk.LEFT, padx=(20, 0))
        self.cfg_iv = tk.Spinbox(r3, from_=0.1, to=5.0, increment=0.1, width=7, font=("微软雅黑", 10), relief=tk.SOLID, bd=1)
        self.cfg_iv.delete(0, tk.END); self.cfg_iv.insert(0, str(self.cfg.get("base_interval", 0.2))); self.cfg_iv.pack(side=tk.LEFT, padx=8)
        r4 = tk.Frame(form, bg=Config.C_CARD); r4.pack(fill=tk.X, pady=8)
        tk.Label(r4, text="深挖并发数：", font=("微软雅黑", 11), bg=Config.C_CARD, fg=Config.C_TEXT, width=13, anchor="e").pack(side=tk.LEFT)
        self.cfg_gw = tk.Spinbox(r4, from_=1, to=5, increment=1, width=7, font=("微软雅黑", 10), relief=tk.SOLID, bd=1)
        self.cfg_gw.delete(0, tk.END); self.cfg_gw.insert(0, str(int(self.cfg.get("grid_workers", 2)))); self.cfg_gw.pack(side=tk.LEFT, padx=8)
        bf = tk.Frame(card, bg=Config.C_CARD); bf.pack(fill=tk.X, padx=14, pady=(4, 14))
        ...
        tip = Card(col_l, title="使用说明", accent=Config.C_INFO); tip.pack(fill=tk.X, pady=6)
        for t in ["• 高德 Key 为必填项，请前往 https://console.amap.com 申请个人开发者 Key",
            ...
            tk.Label(tip, text=t, font=("微软雅黑", 10), bg=Config.C_CARD, fg=Config.C_TEXT_M).pack(anchor="w", padx=15, pady=4)

        # ─────────── 右栏：AI 大模型智能增强（可选） ───────────
        aic = Card(col_r, title="AI 大模型智能增强（可选）", accent=Config.C_INFO); aic.pack(fill=tk.X, pady=0)
        af = tk.Frame(aic, bg=Config.C_CARD); af.pack(fill=tk.X, padx=14, pady=4)
        ...
        ra = tk.Frame(af, bg=Config.C_CARD); ra.pack(fill=tk.X, pady=6)
        tk.Label(ra, text="模型平台：", font=("微软雅黑", 11), bg=Config.C_CARD, fg=Config.C_TEXT, width=13, anchor="e").pack(side=tk.LEFT)
        self.cfg_ai_prov = ttk.Combobox(ra, values=list(Config.AI_PROVIDERS.keys()) + ["custom"], font=("微软雅黑", 10), width=12, state="readonly")
        self.cfg_ai_prov.set(self.cfg.get("ai_provider", "doubao")); self.cfg_ai_prov.pack(side=tk.LEFT, padx=8)
        tk.Label(ra, text="模型：", font=("微软雅黑", 11), bg=Config.C_CARD, fg=Config.C_TEXT).pack(side=tk.LEFT, padx=(8, 0))
        self.cfg_ai_model = tk.Entry(ra, font=("微软雅黑", 10), width=20, relief=tk.SOLID, bd=1); self.cfg_ai_model.pack(side=tk.LEFT, padx=8)
        ...
        rb = tk.Frame(af, bg=Config.C_CARD); rb.pack(fill=tk.X, pady=6)
        tk.Label(rb, text="Base URL：", font=("微软雅黑", 11), bg=Config.C_CARD, fg=Config.C_TEXT, width=13, anchor="e").pack(side=tk.LEFT)
        self.cfg_ai_base = tk.Entry(rb, font=("微软雅黑", 10), width=40, relief=tk.SOLID, bd=1); self.cfg_ai_base.pack(side=tk.LEFT, padx=8, fill=tk.X, expand=True)
        self.cfg_ai_base.insert(0, self.cfg.get("ai_base_url", "") or Config.AI_PROVIDERS.get(self.cfg.get("ai_provider", "doubao"), {}).get("base", ""))
        rc = tk.Frame(af, bg=Config.C_CARD); rc.pack(fill=tk.X, pady=6)
        tk.Label(rc, text="API Key：", font=("微软雅黑", 11), bg=Config.C_CARD, fg=Config.C_TEXT, width=13, anchor="e").pack(side=tk.LEFT)
        self.cfg_ai_key = tk.Entry(rc, font=("微软雅黑", 10), width=40, show="*", relief=tk.SOLID, bd=1); self.cfg_ai_key.pack(side=tk.LEFT, padx=8, fill=tk.X, expand=True)
        self.cfg_ai_key.insert(0, self.cfg.get("ai_api_key", ""))
        tk.Button(rc, text="显示/隐藏", font=("微软雅黑", 10), bg=Config.C_BORDER, fg=Config.C_TEXT, bd=0, padx=8, pady=4,
                  command=lambda ent=self.cfg_ai_key: ent.config(show="" if ent.cget("show") == "*" else "*")).pack(side=tk.LEFT)
        abt = tk.Frame(aic, bg=Config.C_CARD); abt.pack(fill=tk.X, padx=14, pady=(0, 10))
        ...
        for t in ["• 支持 OpenAI 兼容协议的模型平台：豆包（火山方舟）/ DeepSeek / 通义千问 / Kimi / 智谱 GLM / OpenAI 等",
            ...
            tk.Label(aic, text=t, font=("微软雅黑", 10), bg=Config.C_CARD, fg=Config.C_TEXT_M).pack(anchor="w", padx=15, pady=3)
