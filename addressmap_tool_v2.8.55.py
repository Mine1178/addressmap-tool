...
class App:
    ...
    def _build_home(self):
        ...
        for s in ["1. 点击左侧「API 设置」，配置高德/百度 API Key（自动保存到 addr_config.json）",
                  "2. 可选：启用 AI 大模型智能增强，选择平台并填写 Key（未配置时自动使用规则模式）",
                  "3. 准备 Excel 文件，确保包含 Sheet1，按下方表格格式填写数据",
                  "4. 选择「批量正向编码」或「批量逆向编码」，选择文件后开始处理",
                  "5. POI 采集：行政区划或周边检索，自动下钻区县并网格深挖补全，突破单次 255 条上限",
                  "6. 结果自动去重合并，异常/漂移黄色高亮；支持断点续传与失败重试"]:
            ...
            if mode == "district":
                        ...
                        try:
                            ...
                            if d.get("status") != "1":
                                ...
                                if _is_quota_error(info):
                                    self.root.after(0, lambda: self.poi_log.insert(tk.END, "[高德] 配额耗尽（每日调用上限），本次采集提前结束，请次日或更换Key后重试\n", "error"))
                                    self.poi_stop_flag = True
                                    break
                                self.root.after(0, lambda: self.poi_log.insert(tk.END, f"[高德] 错误: {info}\n", "error"))
                                break
                            pois = d.get("pois", [])
                            if d.get("count") and str(d.get("count")).isdigit() and int(d.get("count")) > 225 and not self.poi_break_limit.get():
                                self.root.after(0, lambda c=d.get("count"), un=unit: self.poi_log.insert(tk.END, f"[提示] {un} 共有 {c} 条，超出单参数上限 225 条，建议勾选\"突破限制\"以完整采集\n", "info"))
                            if not pois:
                ...
                while not self.poi_stop_flag and page <= 40:
                    ...
                    try:
                        ...
                        pois = d.get("pois", [])
                        if d.get("count") and str(d.get("count")).isdigit() and int(d.get("count")) > 225 and not self.poi_break_limit.get():
                            self.root.after(0, lambda c=d.get("count"): self.poi_log.insert(tk.END, f"[提示] 周边共 {c} 条，超出单参数上限 225 条，建议勾选\"突破限制\"以完整采集\n", "info"))
                        if not pois:
                            self.root.after(0, lambda p=page: self.poi_log.insert(tk.END, f"[高德] 第{p}页无数据，采集结束\n", "info"))
                            break
                        for p in pois:
                    ...
                    for unit_info in (units if mode == "district" else [{"name": ""}]):
                        ...
                        while not self.poi_stop_flag and ep <= 40:  # 每个扩展词+区县最多40页（1000条）
                            ...
                            try:
                                ed = json.loads(eresp)
                                if ed.get("status") != "1":
                                    if _is_quota_error(ed.get("info", "")):
                                        self.root.after(0, lambda: self.poi_log.insert(tk.END, "[高德] 配额耗尽（每日调用上限），本次采集提前结束\n", "error"))
                                        self.poi_stop_flag = True
                                    break
                                epois = ed.get("pois", [])
                                if not epois: break
                        ...
                        if etruncated and self.poi_break_limit.get():
                            if mode == "district":
    ...
    def _build_settings(self):
        ...
        SButton(abt, "  测试 AI", self._test_ai, "success").pack(side=tk.LEFT)
