...
def grid_collect_poi(gkey, clng, clat, kw, radius, stop_check=None, log_cb=None, poly=None, known=None):
    ...
    known：已采集 POI 列表（含 gcj_lng/gcj_lat）——某点连续 2 页无新增（数据全重复）
    ...
    def scan_point(c_lng, c_lat, r):
        """单点周边 9 页扫描；返回该点是否截断（取满 9 页仍有余量）"""
        truncated = False
        empty_new = 0
        page = 1
        while not stop() and page <= 9:
            ...
            try:
                ...
                for p in pois:
                    ...
                    if poly is not None and (not lng or not lat or not _point_in_polygon(lng, lat, poly)):
                        continue
                    if known_idx.near_any(lng, lat) or got_idx.near_any(lng, lat):
                        continue
    ...
    try:
        ...
        for dx, dy in offsets:
            if stop():
                break
            c_lng, c_lat = clng + dx * dlng, clat + dy * dlat
            truncated = scan_point(c_lng, c_lat, r_scan)
            # 中心点被截断（高密度区域）：半径减半拆 4 子点补扫，突破单点上限
            if truncated and dx == 0 and dy == 0 and r_scan / 2.0 >= 2000:
                sub = r_scan / 2.0
                sdlat = sub / 111000.0
                ...
                for sx, sy in [(0.5, 0.5), (-0.5, 0.5), (0.5, -0.5), (-0.5, -0.5)]:
                    if stop():
                        break
                    scan_point(clng + sx * sdlng * 2, clat + sy * sdlat * 2, sub)
    except _QuotaStop:
        pass
