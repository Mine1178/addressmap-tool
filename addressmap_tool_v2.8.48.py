...
def ai_semantic_dedup(ai, items, max_pairs=200, dist_min=50.0, dist_max=800.0, sim_min=0.55, log_cb=None):
    ...
    for i, j, a, b in cands:
        try:
            same = ai.is_same_entity(a.get("name", ""), b.get("name", ""),
    ...
    def _poi_task(self):
        try:
            ...
            if self.poi_break_limit.get() and len(raw_results) >= 480:
                ...
                main_kw = kw
                expand_kws = []
                # AI 智能关键词扩展优先（未配置 / 调用失败时自动回退静态映射）
                if ai.available:
                    ai_kws = ai.expand_keywords(main_kw)
                    if ai_kws:
                        expand_kws = [ek for ek in ai_kws if ek != main_kw]
                        self.root.after(0, lambda ks=list(expand_kws): self.poi_log.insert(tk.END, "[AI增强] 智能关键词扩展: " + "、".join(ks) + "\n", "info"))
                if not expand_kws:
