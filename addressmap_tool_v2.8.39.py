...
def grid_collect_poi(gkey, clng, clat, kw, radius, stop_check=None, log_cb=None):
                ...
                for unit_info in units:
                    ...
                    while not self.poi_stop_flag and page <= 40:
                        ...
                        try:
                            ...
                            for p in pois:
                                ...
                                if _poi_near_any(raw_results, lng, lat):
                                    continue
                                raw_results.append({
                                    "name": p.get("name", ""), "address": p.get("address", ""),
                                    "tel": p.get("tel", ""), "type": p.get("type", ""),
                    ...
                    if truncated and self.poi_break_limit.get():
                        ...
                        if mclng and mclat:
                            ...
                            grid_pois, grid_calls = grid_collect_poi(gkey, mclng, mclat, kw, 12000, lambda: self.poi_stop_flag)
                            gaode_count += grid_calls
                            added = 0
                            for gp in grid_pois:
                                if not _poi_near_any(raw_results, gp["gcj_lng"], gp["gcj_lat"]):
                    ...
                    for unit_info in (units if mode == "district" else [{"name": ""}]):
                        ...
                        if etruncated and self.poi_break_limit.get():
                            ...
                            if elng and elat:
                                ...
                                epois_g, ecalls = grid_collect_poi(gkey, elng, elat, ek, eradi, lambda: self.poi_stop_flag)
                                gaode_count += ecalls
                                eadd = 0
                                for gp in epois_g:
                                    if not _poi_near_any(raw_results, gp["gcj_lng"], gp["gcj_lat"]):
