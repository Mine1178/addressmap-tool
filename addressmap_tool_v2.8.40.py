...
def get_poi_district_info(gkey, city):
    """通过高德行政区划查询获取城市下属区县信息列表：
    元素 {name, adcode, center, level}；查询失败或无下属区县时回退 [{name: city}]。
    仅保留区县级（level==district），避免直辖市/直筒子市把街道误作检索单位"""
    try:
        ...
        return district_units if district_units else (units if units else [{"name": city}])
    except Exception:
        return [{"name": city}]
