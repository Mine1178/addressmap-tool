...
def grid_collect_poi(gkey, clng, clat, kw, radius, stop_check=None, log_cb=None, poly=None, known=None):
    ...
    def run_points(pts, r):
        ...
        if not pts or quota_event.is_set():
            return results
        workers = min(len(pts), 2)   # 高德实测 2 并发偶发 CUQPS，配合抖动+QPS重试兜底
    ...
    try:
        ...
        for key, val in res.items():
            ...
            # 中心点被截断（高密度区域）：半径减半拆 4 子点补扫（并发），突破单点上限
            if key == (0, 0) and truncated and r_scan / 2.0 >= 2000 and not quota_event.is_set():
                sub = r_scan / 2.0
                sdlat = sub / 111000.0
                ...
                res2 = run_points(sub_pts, sub)
                for val2 in res2.values():
                    if val2 is None:
                        continue
                    calls[0] += val2[1]
                        ...
                        if etruncated and self.poi_break_limit.get():
                            ...
                            if elng and elat:
                                ...
                                epois_g, ecalls = grid_collect_poi(gkey, elng, elat, ek, e_radius, lambda: self.poi_stop_flag,
                                                   log_cb=lambda m: self.root.after(0, lambda mm=m: self.poi_log.insert(tk.END, mm + "\n", "info")), poly=epoly, known=raw_results)
                                gaode_count += ecalls
                                eadd = 0
                                for gp in epois_g:
                                    if not poi_idx.near_any(gp["gcj_lng"], gp["gcj_lat"]):
